...
export class PayterTestStatsBar {
    constructor(game){
        this.game = game; 
        console.log("Using Payter Test Stats Bar")
...
drawSkillNum(context, label, x, y){
 ...
 const flashFrame = TIME_FRAME_KEYS[Number(this.useFlashFrames)];
   ...
   drawMenuStroke(context, x, y, width, height, strokeWidth){
    ...
    drawTestSceneText(context,time){
        this.drawScoreLabel(context, 'PAYTER TEST', 130, 5);
    }
    ...
    draw(context, time){
        ...
        if(this.time===this.timeCount - 0 && this.soundEnable){
        ...
        if (this.time > 0) {
            ...
            } else if (this.time > this.timeCount - 0) {
       ...
       //  this.drawMenu(context, time);
         ...
         if(gameState.pauseMenu.showMoveList)this.drawMoveList(context,time);
          this.drawTestSceneText(context,time);
        }
